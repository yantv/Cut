...
def ffmpeg_run(file, videoFilter, audioFilter, outfile, fps=60, crf=23, codec='libx264', audio_delay=0):
    ...
    command = ["ffmpeg", "-i", file,
               ...
               "-crf", str(crf),
               "-c:v", codec,
               "-itsoffset", str(audio_delay / 1000),  # Apply audio delay if necessary
               "-i", file,  # Input file again for audio delay
               "-map", "0:v", "-map", "1:a",  # Map video from first input and audio from second input
               outfile]
